...
import { PanelPreset, InverterPreset, InverterValidationResult, SystemCompatibility } from '@/types';
import { calculateSeriesConfiguration } from '@/lib/calculations';
...
export default function SystemDesignPage() {
  ...
  const systemAnalysis = useMemo(() => {
    ...
    const seriesResults = calculateSeriesConfiguration(panel, systemConfig.seriesConfig, systemConfig.systemEfficiency);
    ...
    const stringVoltage = seriesResults.voltage;
    ...
    const stringCurrent = seriesResults.current;
    ...
    const compatibility: SystemCompatibility = {
      isCompatible: voltageCompatible && currentCompatible && powerCompatible && vocSafety,
      voltageCompatible,
      currentCompatible,
      powerCompatible,
      issues,
    ...
    const validationResult: InverterValidationResult = {
      panelConfiguration: {
        totalVoltage: stringVoltage,
        totalCurrent: stringCurrent * systemConfig.parallelConfig,
        totalPower: totalSystemPower
      },
      inverterLimits: {
        maxVoltage: inverter.mpptVoltageRange.max,
        maxCurrent: inverter.maxSolarCurrent,
        maxPower: inverter.maxPvPower
      },
      compatibility,
      utilizationFactors: {
            ...
            {systemAnalysis && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      {systemAnalysis.validation.compatibility.isCompatible ? (
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {/* Overall Status */}
                      <div className={`p-4 rounded-lg ${systemAnalysis.validation.compatibility.isCompatible ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className={`font-medium ${systemAnalysis.validation.compatibility.isCompatible ? 'text-green-900' : 'text-red-900'}`}>
                            {systemAnalysis.validation.compatibility.isCompatible ? 'System Compatible' : 'Compatibility Issues Found'}
                          </span>
                        </div>
                        <p className={`text-sm ${systemAnalysis.validation.compatibility.isCompatible ? 'text-green-700' : 'text-red-700'}`}>
                          {systemAnalysis.validation.compatibility.isCompatible ? 
                      ...
                      {/* Issues */}
                      {systemAnalysis.validation.compatibility.issues.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="font-medium text-red-900 flex items-center gap-2">
                            <AlertTriangle className="w-4 h-4" />
                            Critical Issues
                          </h4>
                          {systemAnalysis.validation.compatibility.issues.map((issue, index) => (
                      ...
                      {/* Warnings */}
                      {systemAnalysis.validation.compatibility.warnings.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="font-medium text-yellow-900 flex items-center gap-2">
                            <AlertTriangle className="w-4 h-4" />
                            Warnings
                          </h4>
                          {systemAnalysis.validation.compatibility.warnings.map((warning, index) => (
                      ...
                      {/* Recommendations */}
                      {systemAnalysis.validation.compatibility.recommendations.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="font-medium text-blue-900 flex items-center gap-2">
                            <CheckCircle className="w-4 h-4" />
                            Recommendations
                          </h4>
                          {systemAnalysis.validation.compatibility.recommendations.map((rec, index) => (
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Inverter Utilization</CardTitle>
                    <CardDescription>
                      How well your system utilizes the inverter capacity
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-sm font-medium">Voltage Utilization</span>
                          <span className="text-sm text-gray-600">{systemAnalysis.validation.utilizationFactors.voltage.toFixed(1)}%</span>
                        </div>                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full transition-all duration-300 ${
                              systemAnalysis.validation.utilizationFactors.voltage > 90 ? 'bg-red-500' :
                              systemAnalysis.validation.utilizationFactors.voltage > 80 ? 'bg-yellow-500' :
                              'bg-green-500'
                            }`}
                            style={{ width: `${Math.min(systemAnalysis.validation.utilizationFactors.voltage, 100)}%` }}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-sm text-gray-600">{systemAnalysis.validation.utilizationFactors.current.toFixed(1)}%</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full ${
                              systemAnalysis.validation.utilizationFactors.current > 90 ? 'bg-red-500' :
                              systemAnalysis.validation.utilizationFactors.current > 80 ? 'bg-yellow-500' :
                              'bg-green-500'
                            }`}
                            style={{ width: `${Math.min(systemAnalysis.validation.utilizationFactors.current, 100)}%` }}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-sm text-gray-600">{systemAnalysis.validation.utilizationFactors.power.toFixed(1)}%</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full ${
                              systemAnalysis.validation.utilizationFactors.power > 130 ? 'bg-orange-500' :
                              systemAnalysis.validation.utilizationFactors.power > 100 ? 'bg-yellow-500' :
                              systemAnalysis.validation.utilizationFactors.power > 80 ? 'bg-green-500' :
                              'bg-blue-500'
                            }`}
                            style={{ width: `${Math.min(systemAnalysis.validation.utilizationFactors.power, 150)}%` }}
                          ></div>
                        </div>
                        <p className="text-xs text-gray-500 mt-1">
                          Note: Power oversizing up to 130% is typically acceptable and beneficial
                        </p>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <div className="font-medium">{systemAnalysis.seriesResults.voltage.toFixed(1)}V</div>
                        </div>
                        <div>
                          <span className="text-gray-600">Current (Imp):</span>
                          <div className="font-medium">{systemAnalysis.seriesResults.current.toFixed(1)}A</div>
                        </div>
                        <div>
                          <span className="text-gray-600">Power:</span>
                          <div className="font-medium">{(systemAnalysis.seriesResults.power/1000).toFixed(1)}kW</div>
                        </div>
                        <div>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div>
                          <span className="text-gray-600">Current (Imp):</span>
                          <div className="font-medium">{(systemAnalysis.seriesResults.current * systemConfig.parallelConfig).toFixed(1)}A</div>
                        </div>
                        <div>
                          <span className="text-gray-600">Power:</span>
                          <div className="font-medium">{(systemAnalysis.seriesResults.power * systemConfig.parallelConfig / 1000).toFixed(1)}kW</div>
                        </div>
                        <div>
